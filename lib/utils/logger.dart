...
class Logger {
  ...
  /// Configuración para release - desactiva logs en producción
  static bool get _shouldLog => kDebugMode || level != LogLevel.debug;

  /// Registra un mensaje de depuración
  static void debug(String message) {
    if (_shouldLog && level.index <= LogLevel.debug.index) {
  ...
  static void info(String message) {
    if (_shouldLog && level.index <= LogLevel.info.index) {
  ...
  static void warn(String message) {
    if (_shouldLog && level.index <= LogLevel.warn.index) {
  ...
  static void error(String message) {
    if (_shouldLog && level.index <= LogLevel.error.index) {
...
/// Registra de manera formateada un objeto JSON con código de estado HTTP
void logJson(String label, data,
    {String method = 'DEBUG', String origin = '', int? statusCode}) {
  final encoder = const JsonEncoder();
  final compact = encoder.convert(data);
  ...
  switch (method.toUpperCase()) {
    case 'GET':
      ...
      break;
    case 'POST':
      color = ConsoleColor.post;
      break;
    case 'PUT':
      color = ConsoleColor.put;
      break;
    case 'PATCH':
      color = ConsoleColor.patch;
      break;
    case 'DELETE':
      color = ConsoleColor.delete;
      break;
    default:
      color = ConsoleColor.reset;
  }

  // Color para código de estado
  String statusColor;
  if (statusCode != null) {
    if (statusCode >= 400) {
    ...
    statusColor = ConsoleColor.green; // Por defecto asumimos 200
    statusCode = 200;
  }

  final methodLabel = ConsoleColor.colorize('[$method]', color);
  final statusLabel = ConsoleColor.colorize('[$statusCode]', statusColor);
  final originLabel = origin.isNotEmpty ? ' [$origin]' : '';

  // Formato: [GET] [200] [PedidoApi] Response: {...}
  debugPrint('$methodLabel $statusLabel$originLabel $label: $compact');
}
